...
public class NEMediaController extends FrameLayout {
    ...
    public void setmWindow(PopupWindow mWindow) {
        this.mWindow = mWindow;
    }

    public OnChangeModeListener changeModeListener;


    public OnChangeModeListener getChangeModeListener() {
        return changeModeListener;
    }

    public void setChangeModeListener(OnChangeModeListener changeModeListener) {
    ...
    public void onFinishInflate() {
        if (mRoot != null){
            LogUtil.i("test", "onFinishInflate");
    ...
    protected View makeControllerView() {
     ...
     //   return    LayoutInflater.from(mContext).inflate(R.layout.largecontroller, this);
        return ((LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(R.layout.largecontroller, this);
    }

    private void initControllerView(View v) {
        LogUtil.i("test", "initControllerView");
        ...
        if(mSetPlayerScaleButton != null) {
        	if (mPlayer.isHardware() && mPlayer.isInBackground()) {
				...
				case VIDEO_SCALING_MODE_FIT:
					...
					mSetPlayerScaleButton.setImageResource(R.drawable.img_quanping);
					break;
...
//                    microBar.setVisibility(View.INVISIBLE);
//                    largeBar.setVisibility(View.VISIBLE);
                } else {
                    //  mWindow.getContentView().setVisibility(View.INVISIBLE);
                    mWindow.showAtLocation(mAnchor, Gravity.TOP, anchorRect.left,dp2px(WRStarApplication.getContext(),150)+location[1]);
//                    microBar.setVisibility(View.VISIBLE);
//                    largeBar.setVisibility(View.INVISIBLE);
    ...
    public void hide() {
        ...
        if (mShowing) {
            ...
            try {
                ...
                LogUtil.i(TAG, "MediaController already removed");
            }
...
//            }
    ...
    private long setProgress() {
        ...
        LogUtil.i("getDuration","mDuration:"+mDuration);
        if (mProgress != null) {
            if (duration > 0) {
    ...
    public void updataeProgressEndTime(){
        int duration = mPlayer.getDuration();
             mDuration = duration;
...
//        }
    ...
    private OnClickListener mSetPlayerScaleListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            scaleScreen(mIsFullScreen);
        }
    };

    public void scaleScreen(boolean isfull) {
        if(isfull) {
            mVideoScalingMode = VIDEO_SCALING_MODE_FIT;
            mSetPlayerScaleButton.setImageResource(R.drawable.img_quanping);
            mIsFullScreen = false;
        ...
        try {
            hide();
            changeModeListener.ChangeMode();

            if(mVideoScalingMode == VIDEO_SCALING_MODE_FULL){
