...
import com.wmx.android.wrstar.views.dialog.CloseLiveDialog;
...
/**
 * Displays a VideoPageResponse file. The VideoView class can load images from various
 * sources (such as resources or content providers), takes care of computing its
 * measurement from the VideoPageResponse so that it can be used in any layout manager, and
...
public class NEVideoView extends SurfaceView implements NEMediaController.MediaPlayerControl {
    ...
    public void setVideoScalingMode(int videoScalingMode) {
        Log.i("test_scale", "videoScalingMode" + videoScalingMode);
        ...
        if (mVideoWidth > 0 && mVideoHeight > 0) {
            ...
            if (VIDEO_SCALING_MODE_NONE == videoScalingMode && mSurfaceWidth < winWidth && mSurfaceHeight < winHeight) {
                ...
                /*
                      大小屏修改 - hao
                 */
                layPara.height = DetailVideoActivity.FitVideoHeight;
...
/*
                      大小屏修改 - hao
                 */
                layPara.height = SysUtil.getScreenWidth(WRStarApplication.getContext());
            } else {
                if (winRatio < aspectRatio) {
                    layPara.width = (int) (aspectRatio * winHeight);
                    layPara.height = winHeight;
...
//            Log.d(TAG, "VideoPageResponse: width = " + mVideoWidth + ", height = " + mVideoHeight);
...
//            	neMediaPlayer.setHardwareDecoder(mHardwareDecoder);
            ...
            LogUtil.e(TAG, "Unable to open content: " + mUri + "---ex:" + ex.toString());
    ...
    public void setMediaController(NEMediaController controller, final View v) {
        Log.i("test", "运行了 setMediaController");
        if (mMediaController != null) {
            mMediaController.hide();
        }
        mMediaController = controller;
    ...
    private void attachMediaController() {
        if (mMediaPlayer != null && mMediaController != null) {
            ...
            LogUtil.i("new1", "attachMediaController 5");
    ...
    OnPreparedListener mPreparedListener = new OnPreparedListener() {
        public void onPrepared(NELivePlayer mp) {
            ...
            if (mOnPreparedListener != null) {
                ...
                LogUtil.i("getDuration", "prepared getDuration:" + mMediaPlayer.getDuration() + "");
...
//                                         * at least inform them that the VideoPageResponse is over.
//                                         */
//                                        if (mOnCompletionListener != null)
//                                            mOnCompletionListener.onCompletion(mMediaPlayer);
//                                    }
...
//                        .show();
                ...
                if (mMediaController.ismIsFullScreen()) {
                    Toast.makeText(mContext, "你喜爱的主播，直播到此结束啦，去看看别的直播吧", Toast.LENGTH_SHORT).show();
                } else {
                    CloseLiveDialog dialog = new CloseLiveDialog(mContext);
                    dialog.setClickListener(new CloseLiveDialog.IClickListener() {
                        @Override
                        public void send() {
...
//                                         * at least inform them that the VideoPageResponse is over.
//                                         */
//                                            if (mOnComplet3ionListener != null)
//                                                mOnCompletionListener.onCompletion(mMediaPlayer);
//                                        }
//                                    })
//                            .setCancelable(false)
...
//                    dialog.show();
                ...
                Toast.makeText(mContext, "视频暂时无法打开请稍后再试", Toast.LENGTH_LONG).show();
            }
            return true;
        }
    };
    ...
    public void seekAndChangeUrl(long msec, String path) {
        LogUtil.i("new1", "seekAndChangeUrl 2");
        mUri = Uri.parse(path);
        //mMediaPlayer.stop();
        stopPlayback();
        mSeekWhenPrepared = msec;
