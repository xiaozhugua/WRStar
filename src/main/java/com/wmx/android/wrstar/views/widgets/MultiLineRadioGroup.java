...
public class MultiLineRadioGroup extends ViewGroup implements OnClickListener {
    ...
    public String getCheckKey(){
        if (singleChoice && mLastCheckedPosition >= 0
                && mLastCheckedPosition < viewList.size()) {
              if(map.containsKey(viewList.get(mLastCheckedPosition).getText().toString())){
                  return    map.get(viewList.get(mLastCheckedPosition).getText().toString());
              }else{
                  return "999";
              }
    ...
    public int append(String str,String id) {
        CheckBox cb = getChild();
        cb.setText(str);
